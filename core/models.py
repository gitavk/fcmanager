...
class User(AbstractUser):
    ...
    def reminders(self):
        reminders = Reminder.objects.filter(is_read=False,
                                            is_everyday=True,
                                            time__lte=datetime.now().time())
        reminders = reminders | Reminder.objects.filter(is_read=False,
                                            date__lte=datetime.now().date(),
                                            time__lte=datetime.now().time())

        wday = datetime.now().weekday()

        for r in Reminder.objects.filter(is_read=False,
                                        is_everyday=False,
                                        date__isnull=True):
