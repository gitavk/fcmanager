...
def shift(request, ):
	...
	context_dict = dict(request=request, b_url=b_url,
					shifts=shifts)
...
def timetable(request, year=0, week=0, act=None):
	...
	context_dict = dict(request=request, week=week, dates=dates,
					shifts=shifts, year=year, lastw=lastw,
					lastw_b=lastw_b, employees=employees,
					act=act,)
	if act == 'edit':
		context_dict.update(csrf(request))
	return render_to_response('timetable.html', context_dict)
	# return HttpResponse(week,mimetype='text/html')
...
def dismiss(request, ):
	...
	cnt = WorkRecord.objects.filter(employee=e, 
					date_end__isnull=True).update(date_end=datetime.now())
...
def change_work(request, id=0):
	...
	if request.method == 'POST':
		...
		if wf.is_valid():
			WorkRecord.objects.filter(employee=e, date_end__isnull=True).update(date_end=datetime.now())
			wf.save()
			return HttpResponseRedirect(b_url)
		else:
			errors = wf.errors
	...
	if act == 'add':
		...
		if act == 'del':
			...
			if request.method == 'POST':
				...
				return HttpResponseRedirect(b_url)
			positions = Position.objects.all()
			dep_positions = DepartmentPosition.objects.filter(department=d).values_list('position', flat=True)
			context_dict = dict(request=request, p_title=p_title, edit=1, b_url=b_url,
