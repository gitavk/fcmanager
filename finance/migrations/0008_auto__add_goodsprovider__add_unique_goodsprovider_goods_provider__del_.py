...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Goods', fields ['name', 'provider']
        db.delete_unique(u'finance_goods', ['name', 'provider_id'])

        # Adding model 'GoodsProvider'
        db.create_table(u'finance_goodsprovider', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('goods', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['finance.Goods'])),
            ('provider', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['finance.Provider'])),
        ))
        db.send_create_signal(u'finance', ['GoodsProvider'])

        # Adding unique constraint on 'GoodsProvider', fields ['goods', 'provider']
        db.create_unique(u'finance_goodsprovider', ['goods_id', 'provider_id'])

        # Deleting field 'Goods.provider'
        db.delete_column(u'finance_goods', 'provider_id')

        # Adding unique constraint on 'Goods', fields ['name']
        db.create_unique(u'finance_goods', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Goods', fields ['name']
        db.delete_unique(u'finance_goods', ['name'])

        # Removing unique constraint on 'GoodsProvider', fields ['goods', 'provider']
        db.delete_unique(u'finance_goodsprovider', ['goods_id', 'provider_id'])

        # Deleting model 'GoodsProvider'
        db.delete_table(u'finance_goodsprovider')

        # Adding field 'Goods.provider'
        db.add_column(u'finance_goods', 'provider',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['finance.Provider'], null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'Goods', fields ['name', 'provider']
        db.create_unique(u'finance_goods', ['name', 'provider_id'])
    ...
    models = {
        ...
        u'finance.creditshistory': {
            ...
            'payment_user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['core.User']"})
        },
        u'finance.goods': {
