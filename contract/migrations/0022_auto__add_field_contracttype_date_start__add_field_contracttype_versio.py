...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'ContractType', fields ['name']
        db.delete_unique(u'contract_contracttype', ['name'])

        # Adding field 'ContractType.date_start'
        db.add_column(u'contract_contracttype', 'date_start',
                      self.gf('django.db.models.fields.DateTimeField')(auto_now=True, default=datetime.datetime(2014, 4, 29, 0, 0), blank=True),
                      keep_default=False)

        # Adding field 'ContractType.version'
        db.add_column(u'contract_contracttype', 'version',
                      self.gf('django.db.models.fields.IntegerField')(default=0),
                      keep_default=False)

        # Adding unique constraint on 'ContractType', fields ['name', 'version']
        db.create_unique(u'contract_contracttype', ['name', 'version'])


    def backwards(self, orm):
        # Removing unique constraint on 'ContractType', fields ['name', 'version']
        db.delete_unique(u'contract_contracttype', ['name', 'version'])

        # Deleting field 'ContractType.date_start'
        db.delete_column(u'contract_contracttype', 'date_start')

        # Deleting field 'ContractType.version'
        db.delete_column(u'contract_contracttype', 'version')

        # Adding unique constraint on 'ContractType', fields ['name']
        db.create_unique(u'contract_contracttype', ['name'])
